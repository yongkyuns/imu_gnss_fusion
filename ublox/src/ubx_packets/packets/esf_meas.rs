    ...
    fn next(&mut self) -> Option<Self::Item> {
        match self {
            Self::Bytes(iter) => {
                ...
                let data_type: EsfSensorType = (((data >> 24) & 0x3F) as u8).into();
                // ESF-MEAS payload uses signed 24-bit sensor data in the low 24 bits.
                let raw24 = (data & 0x00FF_FFFF) as i32;
                let mut data_field = raw24;
                if (data_field & 0x0080_0000) != 0 {
                    data_field -= 1 << 24;
                }
