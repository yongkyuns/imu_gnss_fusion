...
\texttt{sensor_fusion/src/rust\_reduced.rs}, \texttt{sensor_fusion/src/generated\_reduced.rs}, and the
...
\end{equation}
If \texttt{MountSource::LatchedSeed} is selected, \(q_{vb}^{0}\) is frozen
at Reduced initialization. If \texttt{MountSource::FollowAlign} is selected, the pre-rotation
...
The Reduced consumes one-sample IMU increments already expressed in the seeded
frame:
\begin{equation}
...
when the Reduced initializes. Residual mount \(q_{cs}\) may then estimate remaining
seed error.
...
Reduced should not own mount estimation.
