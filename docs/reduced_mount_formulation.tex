...
\title{Augmented Reduced Mathematical Formulation}
...
This document describes the runtime augmented Reduced implemented by
\path{sensor_fusion/src/reduced/} and the included generated model files. The
filter estimates local-NED vehicle attitude, velocity, position, IMU biases, and
the physical vehicle-to-body mount used during inertial propagation.
...
\midrule
\(n\) & Local NED frame. Reduced position and velocity states are in this frame. \\
...
layout this mount is stored in \texttt{qcs0..qcs3}, but its mathematical meaning
is \(q_{bv}\):
\begin{equation}
  x_b=C_{bv}(q_{bv})x_v,\qquad x_v=C_{bv}(q_{bv})\T x_b .
\end{equation}
If \texttt{MountSource::LatchedSeed} is selected, Align seeds \(q_{bv}\) and
Reduced continues estimating it. If \texttt{MountSource::FollowAlign} is
selected, the mount follows Align and Reduced mount states are frozen. A fixed
external mount is also supported.
...
The Reduced consumes one-sample raw body-frame IMU increments:
\begin{equation}
...
\begin{equation}
  \Delta\theta_v^c=C_{bv}(q_{bv})\T\Delta\theta_b^c,\qquad
  \Delta v_v^c=C_{bv}(q_{bv})\T\Delta v_b^c .
\end{equation}
The generated nominal equations implement
\begin{align}
  q_{nv}^{+} &= \operatorname{normalize}\left(q_{nv}^{-}\otimes
  \dq(\Delta\theta_v^c)\right),\\
  v_n^{+} &= v_n^{-}+C_{nv}(q_{nv}^{+})\Delta v_v^c+g_n\dt,\\
  p_n^{+} &= p_n^{-}+v_n^{+}\dt,\\
...
biases to zero, and initializes \(q_{bv}\) from Align, reference data, or an
external mount. Covariance is diagonal with configuration-controlled bias and
mount variances.

The fusion facade controls the mount source:
\begin{itemize}[leftmargin=1.5em]
\item \textbf{Latched/internal}: Align seeds \(q_{bv}\) when Reduced initializes;
Reduced may then continue estimating mount.
\item \textbf{FollowAlign}: \(q_{bv}\) tracks Align and Reduced mount states are
frozen. This is useful when Reduced should not own mount estimation.
