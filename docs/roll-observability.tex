...
error-state EKF using the project's Reduced NED convention. The same local
analysis applies to the Full ECEF filter after projecting ECEF velocity and
...
The local EKF convention is the Reduced convention:
\begin{equation}
...
\end{equation}
Full uses \(v_v=C_{ev}\T v_e\), but once this projection is made the local
roll/pitch ambiguity has the same structure. Therefore the rest of the document
uses the Reduced NED notation.
...
observations in the local Reduced basis and accumulate
\begin{equation}
...
and then runs the Reduced EKF with two covariance splits:
\begin{itemize}[nosep]
  \item \texttt{qerr\_mnt\_loose}: tighter vehicle-attitude covariance and
        looser mount covariance,
  \item \texttt{qerr\_att\_loose}: looser vehicle-attitude covariance and
