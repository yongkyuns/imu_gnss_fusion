...
Align seeds \(q_{bv}\) and Reduced continues estimating mount internally. In
\texttt{MountMode::Manual}, the caller supplies a fixed \(q_{bv}\) and Reduced
...
\item \textbf{Auto}: Align seeds \(q_{bv}\) when Reduced initializes; Reduced
then estimates mount states internally.
\item \textbf{Manual}: a supplied or reference \(q_{bv}\) is used as a fixed
mount and Reduced mount states are frozen.
\end{itemize}
