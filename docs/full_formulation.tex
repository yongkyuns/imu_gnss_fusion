...
\texttt{sensor_fusion/src/full/} and \texttt{sensor_fusion/src/full/generated.rs}. It is used
for seeded replay, diagnostics, and comparison with the runtime augmented Reduced.
...
\(s\) & Seeded sensor/vehicle frame of the IMU deltas consumed by the full filter. \\
...
\end{equation}
The full attitude quaternion \(q_{es}\) maps seeded-frame vectors into ECEF:
\begin{equation}
  x_e=C_{es}(q_{es})x_s .
\end{equation}
...
The full NHC predicts corrected vehicle-frame velocity from ECEF velocity:
\begin{equation}
...
\end{align}
Both quaternions are normalized. After injection, the attitude and residual
mount covariance blocks are reset with the generated first-order quaternion
reset Jacobian
\begin{equation}
  G_r(\delta\theta)\simeq I-\frac{1}{2}\crossmat{\delta\theta}.
\end{equation}
The reset is applied to the \(q_{es}\) attitude block and to the \(q_{cs}\)
residual-mount block, then the covariance is symmetrized.
...
initializes \(q_{cs}=1\). This lets the full path consume the same
pre-rotated IMU stream used by Reduced comparisons while propagating navigation in
ECEF.
