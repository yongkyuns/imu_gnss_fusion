...
the inertial propagation model for both the runtime Reduced and the full
Full filter. It is intentionally a before/after document. The Full
steady-state formulations remain in the dedicated Reduced and full formulation
...
\(n\) & Local NED navigation frame used by the Reduced. \\
...
The old Reduced nominal state was
\begin{equation}
  x_{\mathrm{old}} =
  \begin{bmatrix}
...
The full filter had the same structure in ECEF:
\begin{equation}
  x_{\mathrm{old,full}} =
  \begin{bmatrix}
...
the Reduced or full propagation step, the runtime converted raw IMU samples into
the seeded frame:
\begin{align}
...
\subsection{Old Reduced Propagation}

The old Reduced prediction was effectively
\begin{align}
...
The revised Reduced keeps the same state dimension and Rust field layout, but
...
The revised Reduced nominal state is
\begin{equation}
  x_{\mathrm{new}} =
  \begin{bmatrix}
...
The revised Reduced propagation is
\begin{align}
  q_{nv}^{+} &=
...
For full, the same structure is used with ECEF gravity and Earth rotation
terms:
\begin{align}
...
\subsection{Generated Reduced}

The symbolic source is \path{sensor_fusion/src/reduced/formulation.py}. The generated Rust fragments are
included by \path{sensor_fusion/src/reduced/generated.rs}. The important changes are:
...
\subsection{Runtime Reduced Facade}

\texttt{sensor_fusion/src/fusion.rs} no longer pre-rotates IMU before calling
\texttt{reduced::Filter::predict}. It initializes \texttt{nominal.qcs*} from the align
...
\subsection{Full Reference Filter}

The symbolic source is \texttt{sensor_fusion/src/full/formulation.py}. The generated Rust
fragments are included by \texttt{sensor_fusion/src/full/generated.rs}. The important
changes are:
\begin{itemize}[leftmargin=*]
  \item The transition matrix uses \texttt{qcs} as \(q_{vb}\) in the IMU
        rotation terms.
  \item Full nominal propagation in \texttt{sensor_fusion/src/full/} rotates raw
        body-frame gyro and accelerometer measurements into the vehicle frame.
  \item Full NHC rows observe \(C_{ev}\T v_e\) and have no direct mount
        support.
  \item The visualizer seeds full \texttt{qcs} with align mount and plots it
...
split. This matters most for the full filter because it initializes vehicle
attitude from a yaw-only state. If the full vehicle-attitude covariance is too
...
\begin{equation}
  \sigma_{\theta,\mathrm{full}}=2^\circ
\end{equation}
to
\begin{equation}
  \sigma_{\theta,\mathrm{full}}=20^\circ.
\end{equation}
This is not an ad-hoc correction to one maneuver. It reflects the weaker
initial information content of the full yaw-only attitude seed.
...
\midrule
Reduced & approximately \(0.46^\circ\) & \(0.0277^\circ\) \\
Full & approximately \(0.45^\circ\) & \(0.0321^\circ\) \\
\bottomrule
\end{tabular}
\end{center}
...
\end{tabular}
\end{center}
In that same faulted case, the full GNSS position gate accepted all GNSS
position updates after the revision. Before this change, the bad state split
could drive the full prediction far enough away that the full 3D position gate
...
\begin{itemize}[leftmargin=*]
  \item \texttt{sensor_fusion/tests/reduced\_nhc\_jacobian.rs}: finite-difference check for
        the direct vehicle-frame Reduced NHC Jacobians.
  \item \texttt{sensor_fusion/tests/nhc\_reduced\_full\_equivalence.rs}: verifies Reduced and
        full NHC rows and updates remain equivalent after the attitude basis
        transform.
  \item \texttt{sensor_fusion/tests/full\_mount\_observability.rs}: verifies NHC does
...
The existing Reduced and full formulation PDFs still describe portions of the old
