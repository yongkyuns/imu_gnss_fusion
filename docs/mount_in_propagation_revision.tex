...
The symbolic source is \path{sensor_fusion/reduced.py}. The generated Rust fragments are
included by \path{sensor_fusion/src/generated_reduced.rs}. The important changes are:
...
\texttt{RustReduced::predict}. It initializes \texttt{nominal.qcs*} from the align
...
The symbolic source is \texttt{sensor_fusion/ins\_gnss\_full.py}. The generated Rust
fragments are included by \texttt{sensor_fusion/src/generated\_full.rs}. The important
changes are:
\begin{itemize}[leftmargin=*]
  \item The transition matrix uses \texttt{qcs} as \(q_{vb}\) in the IMU
        rotation terms.
  \item Full nominal propagation in \texttt{sensor_fusion/src/full.rs} rotates raw
