...
static void sf_mat3_mul(const float a[3][3], const float b[3][3],
                        float out[3][3]);
static void sf_lla_to_ecef(double lat_deg, double lon_deg, double height_m,
                           double out_ecef_m[3]);
static void sf_ecef_to_ned_matrix(float lat_deg, float lon_deg,
                                  float out[3][3]);
...
static float
sf_heading_local_to_anchor(const sf_internal_anchor_state_t *anchor,
                           float lat_deg, float lon_deg,
                           float heading_local_rad);
static float sf_body_speed_x_estimate(const sf_eskf_t *eskf);
static void
sf_rotate_nav_state_to_new_anchor(sf_sensor_fusion_impl_t *impl,
                                  const sf_internal_anchor_state_t *new_anchor);
static void sf_rotate_eskf_covariance_nav_blocks(sf_eskf_t *eskf,
                                                 const float r_n1_n0[3][3]);
...
void sf_fusion_set_misalignment(sf_sensor_fusion_t *fusion,
  ...
  impl->eskf.nominal.qcs3 = 0.0f;
}

void sf_fusion_set_r_body_vel(sf_sensor_fusion_t *fusion, float r_body_vel) {
  sf_sensor_fusion_impl_t *impl;
  if (fusion == NULL || !isfinite(r_body_vel) || r_body_vel < 0.0f) {
    return;
  }
  impl = sf_impl(fusion);
...
sf_update_t sf_fusion_process_imu(sf_sensor_fusion_t *fusion,
  ...
  const float *predict_mount_q_vb;
  ...
  predict_mount_q_vb =
      impl->eskf_mount_q_vb_valid ? impl->eskf_mount_q_vb : impl->mount_q_vb;
  sf_quat_to_rotmat(predict_mount_q_vb, c_bv);
  ...
  if (impl->cfg.r_body_vel > 0.0f) {
    zero_vel_active = sf_runtime_zero_velocity_active(impl, sample->accel_mps2,
                                                      sample->gyro_radps);
    t0_us = sf_profile_stamp(impl);
    if (zero_vel_active) {
...
sf_update_t sf_fusion_process_gnss(sf_sensor_fusion_t *fusion,
  ...
  if (!impl->ekf_initialized) {
    if (impl->mount_q_vb_valid) {
      memcpy(impl->eskf_mount_q_vb, impl->mount_q_vb,
             sizeof(impl->eskf_mount_q_vb));
      impl->eskf_mount_q_vb_valid = true;
    }
    t0_us = sf_profile_stamp(impl);
...
sf_update_t
sf_fusion_process_vehicle_speed(sf_sensor_fusion_t *fusion,
                                const sf_vehicle_speed_sample_t *sample) {
  sf_sensor_fusion_impl_t *impl;
...
static void sf_initialize_eskf_from_gnss(sf_eskf_t *eskf,
  ...
  eskf->p[15][15] = 0.0f;
  eskf->p[16][16] = mount_residual_sigma_rad * mount_residual_sigma_rad;
  eskf->p[17][17] = mount_residual_sigma_rad * mount_residual_sigma_rad;
}
...
static void sf_anchor_set_from_lla(sf_internal_anchor_state_t *anchor,
  ...
  sf_lla_to_ecef((double)lat_deg, (double)lon_deg, (double)height_m,
                 anchor->ecef_m);
...
sf_heading_local_to_anchor(const sf_internal_anchor_state_t *anchor,
                           float lat_deg, float lon_deg,
                           float heading_local_rad) {
  float forward_local[3] = {cosf(heading_local_rad), sinf(heading_local_rad),
                            0.0f};
  float c_ne_local[3][3];
  float c_en_local[3][3];
...
static void sf_rotate_nav_state_to_new_anchor(
    sf_sensor_fusion_impl_t *impl,
    const sf_internal_anchor_state_t *new_anchor) {
  float c_en_old[3][3];
  float r_n1_n0[3][3];
  ...
  float q_old[4];
  float q_new[4];
  if (impl == NULL || !impl->anchor.valid || new_anchor == NULL ||
      !new_anchor->valid) {
    return;
  }
  ...
  if (impl->ekf_initialized) {
    float p_n0[3] = {impl->eskf.nominal.pn, impl->eskf.nominal.pe,
                     impl->eskf.nominal.pd};
    float v_old[3] = {impl->eskf.nominal.vn, impl->eskf.nominal.ve,
                      impl->eskf.nominal.vd};
    p_ecef[0] = impl->anchor.ecef_m[0] + c_en_old[0][0] * p_n0[0] +
                c_en_old[0][1] * p_n0[1] + c_en_old[0][2] * p_n0[2];
    p_ecef[1] = impl->anchor.ecef_m[1] + c_en_old[1][0] * p_n0[0] +
                c_en_old[1][1] * p_n0[1] + c_en_old[1][2] * p_n0[2];
    p_ecef[2] = impl->anchor.ecef_m[2] + c_en_old[2][0] * p_n0[0] +
                c_en_old[2][1] * p_n0[1] + c_en_old[2][2] * p_n0[2];
  ...
  if (impl->last_gnss_valid) {
    double last_ecef[3];
    float pos_old[3] = {impl->last_gnss.pos_ned_m[0],
                        impl->last_gnss.pos_ned_m[1],
                        impl->last_gnss.pos_ned_m[2]};
    v_n0[0] = impl->last_gnss.vel_ned_mps[0];
    v_n0[1] = impl->last_gnss.vel_ned_mps[1];
    v_n0[2] = impl->last_gnss.vel_ned_mps[2];
    last_ecef[0] = impl->anchor.ecef_m[0] + c_en_old[0][0] * pos_old[0] +
                   c_en_old[0][1] * pos_old[1] + c_en_old[0][2] * pos_old[2];
    last_ecef[1] = impl->anchor.ecef_m[1] + c_en_old[1][0] * pos_old[0] +
                   c_en_old[1][1] * pos_old[1] + c_en_old[1][2] * pos_old[2];
    last_ecef[2] = impl->anchor.ecef_m[2] + c_en_old[2][0] * pos_old[0] +
                   c_en_old[2][1] * pos_old[1] + c_en_old[2][2] * pos_old[2];
...
static bool sf_prepare_local_gnss_sample(sf_sensor_fusion_impl_t *impl,
  ...
  if (!impl->anchor.valid) {
    sf_anchor_set_from_lla(&impl->anchor, sample_in->lat_deg,
                           sample_in->lon_deg, sample_in->height_m);
  }
  ...
  horiz_dist_m =
      sqrtf(pos_ned_m[0] * pos_ned_m[0] + pos_ned_m[1] * pos_ned_m[1]);
  ...
  sf_velocity_local_ned_to_anchor_ned(
      &impl->anchor, sample_in->lat_deg, sample_in->lon_deg,
      sample_in->vel_ned_mps, vel_anchor_ned_mps);
  memset(sample_out, 0, sizeof(*sample_out));
  sample_out->t_s = sample_in->t_s;
  memcpy(sample_out->pos_ned_m, pos_ned_m, sizeof(pos_ned_m));
  memcpy(sample_out->vel_ned_mps, vel_anchor_ned_mps,
         sizeof(vel_anchor_ned_mps));
  memcpy(sample_out->pos_std_m, sample_in->pos_std_m,
         sizeof(sample_out->pos_std_m));
  memcpy(sample_out->vel_std_mps, sample_in->vel_std_mps,
         sizeof(sample_out->vel_std_mps));
  sample_out->heading_valid = sample_in->heading_valid;
  sample_out->heading_rad =
      sample_in->heading_valid
...
static float sf_body_speed_x_estimate(const sf_eskf_t *eskf) {
  ...
  return (1.0f - 2.0f * qcs2 * qcs2 - 2.0f * qcs3 * qcs3) * vs0 +
...
static bool sf_runtime_zero_velocity_active(sf_sensor_fusion_impl_t *impl,
  ...
  low_dynamic = gyro_ema <= impl->cfg.bootstrap.max_gyro_radps &&
                accel_ema <= impl->cfg.bootstrap.max_accel_norm_err_mps2;
  speed_mps = impl->last_gnss_valid
                  ? sf_horiz_speed(impl->last_gnss.vel_ned_mps)
                  : 0.0f;
