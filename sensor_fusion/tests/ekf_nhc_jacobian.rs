use sensor_fusion::ekf::NominalState;
use sensor_fusion::generated_ekf::{self, ERROR_STATES};
...
fn body_velocity_yz_jacobians_match_nominal_model_finite_difference() {
    ...
    let obs_y = generated_ekf::body_vel_y_observation(&nominal, &p, 1.0);
    let obs_z = generated_ekf::body_vel_z_observation(&nominal, &p, 1.0);
