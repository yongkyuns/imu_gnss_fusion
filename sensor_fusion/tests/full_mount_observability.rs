use sensor_fusion::ProcessNoise;
use sensor_fusion::full::{FULL_ERROR_STATES, FullFilter};

fn initialized_full(vel_ecef_mps: [f32; 3]) -> FullFilter {
    let mut p_diag = [1.0e-4; FULL_ERROR_STATES];
    p_diag[3] = 1.0;
    p_diag[4] = 1.0;
    p_diag[5] = 1.0;
    p_diag[21] = 0.25;
    p_diag[22] = 0.25;
    p_diag[23] = 0.25;

    let mut full = FullFilter::new(ProcessNoise::reference_nsr_demo());
    full.init_from_reference_state(
        [1.0, 0.0, 0.0, 0.0],
        [6_378_137.0, 0.0, 0.0],
        vel_ecef_mps,
        [0.0; 3],
        [0.0; 3],
        [1.0; 3],
        [1.0; 3],
        [1.0, 0.0, 0.0, 0.0],
        Some(p_diag),
    );
    full
}

#[test]
fn weak_nhc_direct_mount_sensitivity_does_not_move_mount() {
    let mut full = initialized_full([0.01, 0.005, 0.0]);

    full.fuse_nhc_reference([0.0; 3], [0.0, 0.0, 9.81], 0.02);

    assert_eq!(full.last_dx()[21], 0.0);
    assert_eq!(full.last_dx()[22], 0.0);
    assert_eq!(full.last_dx()[23], 0.0);
}

#[test]
fn moving_nhc_updates_attitude_but_not_mount_directly() {
    let mut full = initialized_full([5.0, 1.0, 0.0]);

    full.fuse_nhc_reference_with_speed(Some(5.0), [0.0; 3], [0.0, 0.0, 9.81], 0.02);

    let dx_att = &full.last_dx()[6..9];
    let att_max_abs = dx_att
        .iter()
        .fold(0.0_f32, |acc, value| acc.max(value.abs()));
    assert!(
        att_max_abs > 1.0e-6,
        "expected observable moving NHC update to move attitude, dx={dx_att:?}"
    );
    let dx_mount = &full.last_dx()[21..24];
    assert!(
        dx_mount.iter().all(|v| *v == 0.0),
        "NHC no longer has direct mount sensitivity; dx={dx_mount:?}"
    );
}

#[test]
fn batched_nhc_uses_reference_velocity_speed_by_default() {
    let mut full = initialized_full([5.0, 1.0, 0.0]);

    full.fuse_reference_batch_full(
        None,
        Some([5.0, 1.0, 0.0]),
        1.0,
        None,
        0.02,
        [0.0; 3],
        [0.0, 0.0, 9.81],
        0.02,
    );

    assert!(full.last_obs_types().contains(&7));
    assert!(full.last_obs_types().contains(&8));
}

#[test]
fn batched_nhc_without_reference_speed_does_not_fall_back_to_state_speed() {
    let mut full = initialized_full([5.0, 1.0, 0.0]);

    full.fuse_reference_batch_full(
        None,
        None,
        1.0,
        None,
        0.02,
        [0.0; 3],
        [0.0, 0.0, 9.81],
        0.02,
    );

    assert!(full.last_obs_types().is_empty());
    assert_eq!(full.last_dx()[21], 0.0);
    assert_eq!(full.last_dx()[22], 0.0);
    assert_eq!(full.last_dx()[23], 0.0);
}

#[test]
fn external_zero_speed_suppresses_batched_nhc_even_when_state_speed_is_nonzero() {
    let mut full = initialized_full([5.0, 1.0, 0.0]);

    full.fuse_reference_batch_full_with_nhc_speed(
        None,
        None,
        1.0,
        None,
        0.02,
        Some(0.0),
        [0.0; 3],
        [0.0, 0.0, 9.81],
        0.02,
    );

    assert!(full.last_obs_types().is_empty());
    assert_eq!(full.last_dx()[21], 0.0);
    assert_eq!(full.last_dx()[22], 0.0);
    assert_eq!(full.last_dx()[23], 0.0);
}

#[test]
fn external_moving_speed_allows_batched_nhc() {
    let mut full = initialized_full([5.0, 1.0, 0.0]);

    full.fuse_reference_batch_full_with_nhc_speed(
        None,
        None,
        1.0,
        None,
        0.02,
        Some(5.0),
        [0.0; 3],
        [0.0, 0.0, 9.81],
        0.02,
    );

    assert!(full.last_obs_types().contains(&7));
    assert!(full.last_obs_types().contains(&8));
}

#[test]
fn caller_provided_nhc_r_reduces_batched_attitude_correction() {
    let mut default_r = initialized_full([5.0, 1.0, 0.0]);
    default_r.fuse_reference_batch_full_with_nhc_speed(
        None,
        None,
        1.0,
        None,
        0.02,
        Some(5.0),
        [0.0; 3],
        [0.0, 0.0, 9.81],
        0.02,
    );

    let mut matched_r = initialized_full([5.0, 1.0, 0.0]);
    matched_r.fuse_reference_batch_full_with_nhc_speed_and_r(
        None,
        None,
        1.0,
        None,
        0.02,
        Some(5.0),
        0.5,
        0.125,
        [0.0; 3],
        [0.0, 0.0, 9.81],
        0.02,
    );

    let default_att_abs: f32 = default_r.last_dx()[6..9].iter().map(|v| v.abs()).sum();
    let matched_att_abs: f32 = matched_r.last_dx()[6..9].iter().map(|v| v.abs()).sum();

    assert!(
        matched_att_abs < default_att_abs * 0.25,
        "expected larger caller-provided NHC R to reduce attitude correction, default={default_att_abs}, matched={matched_att_abs}"
    );
}
