...
impl Filter {
    ...
    /// Returns the raw EKF state.
    pub fn raw(&self) -> &State {
        &self.raw
    }

    /// Returns mutable access to the raw EKF state for integration code and diagnostics.
    ...
    ///
    /// All rows are linearized at the same nominal state, one error state is
    /// solved, and the nominal is injected once. Running this at GNSS rate keeps
    /// the cost bounded while preserving the important GNSS/NHC covariance
    /// allocation.
