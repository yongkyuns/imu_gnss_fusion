...
use crate::math::{asin_f32, atan2_f32, ceil_f32, cos_f32, sin_f32, sq_f32, sqrt_f32};
...
impl Align {
    ...
    pub fn update_window_with_trace(
        ...
        let course_prev = atan2_f32(v_prev[1], v_prev[0]);
        let course_curr = atan2_f32(v_curr[1], v_curr[0]);
        ...
        let horiz_gnss_norm = sqrt_f32(a_long * a_long + a_lat * a_lat);
        ...
        let horiz_imu_norm = sqrt_f32(horiz_obs[3] * horiz_obs[3] + horiz_obs[4] * horiz_obs[4]);
    ...
    pub fn sigma_deg(&self) -> [f32; 3] {
        [
            sqrt_f32(self.P[0][0].max(0.0)).to_degrees(),
            sqrt_f32(self.P[1][1].max(0.0)).to_degrees(),
            sqrt_f32(self.P[2][2].max(0.0)).to_degrees(),
        ]
    }
    ...
    fn turn_consistency_update(
        ...
        let min_ok = ceil_f32(fraction * self.turn_count as f32) as usize;
...
fn quat_to_rotmat(q: [f32; 4]) -> [[f32; 3]; 3] {
    ...
    let inv = if n2 > 1.0e-9 { 1.0 / sqrt_f32(n2) } else { 1.0 };
    let q0 = q[0] * inv;
    let q1 = q[1] * inv;
    let q2 = q[2] * inv;
    let q3 = q[3] * inv;
...
fn rot_to_euler_zyx(r: [[f32; 3]; 3]) -> [f32; 3] {
    let pitch = asin_f32((-r[2][0]).clamp(-1.0, 1.0));
    let roll = atan2_f32(r[2][1], r[2][2]);
    let yaw = atan2_f32(r[1][0], r[0][0]);
    [roll, pitch, yaw]
}
