function tmp_loose_matlab_outage_eval()
...
if evalin('base', 'exist(''outageWindowsS'',''var'')')
    outageWindowsS = evalin('base', 'outageWindowsS');
else
    outageWindowsS = zeros(0, 2);
end
...
applied_obs_types = cell(length(gyroData), 1);
count = 0;
...
for i = 1:length(order)
    switch allEvents(order(i))
        case 1
            ...
            [obsDb, H, obsTypes] = prep_obs_with_outage(x, P, filTtag, lastUsedGnssTtag, measDb, conf, sMap, esMap, init.start_ttag_us, outageWindowsS);
            ...
            q_es(count, :) = x(sMap.Q_ES)';
            q_cs(count, :) = x(sMap.Q_CS)';
            applied_obs_types{count} = canonical_obs_types(obsTypes);
        case 2
            measDb = measDb.addData(accelData(nextAccel), SensorType.ACCEL);
            nextAccel = nextAccel + 1;
        case 3
            if ~is_outage_time(double(gnssData(nextGnss).ttag - uint64(init.start_ttag_us)) * 1e-6, outageWindowsS)
...
out.applied_obs_types = {applied_obs_types{1:count}};
out.outage_windows_s = outageWindowsS;
...
function tf = is_outage_time(t_s, outageWindowsS)
tf = false;
for i = 1:size(outageWindowsS, 1)
    if t_s >= outageWindowsS(i, 1) && t_s <= outageWindowsS(i, 2)
        tf = true;
        return;
    end
end
...
function [obsDb, H, obsTypes] = prep_obs_with_outage(x, P, filTtag, lastUsedGnssTtag, measDb, conf, sMap, esMap, startTtagUs, outageWindowsS)
obsDb = ObsDb(20);
H = zeros(20, esMap.LEN);
obsTypes = zeros(1, 0);

[obsDb, H, gpsTypes] = prep_gnss_update_with_outage(obsDb, H, x, P, filTtag, lastUsedGnssTtag, measDb, sMap, esMap, startTtagUs, outageWindowsS);
obsTypes = [obsTypes, gpsTypes]; %#ok<AGROW>
[obsDb, H, nhcTypes] = prep_nhc_update_with_types(obsDb, H, x, P, measDb, conf, sMap, esMap);
obsTypes = [obsTypes, nhcTypes]; %#ok<AGROW>
end

function [obsDb, H, obsTypes] = prep_gnss_update_with_outage(obsDb, H, x, P, filTtag, lastUsedGnssTtag, measDb, sMap, esMap, startTtagUs, outageWindowsS)
obsTypes = zeros(1, 0);
...
t_s = 1e-6 * double(gnssData.ttag - uint64(startTtagUs));
if is_outage_time(t_s, outageWindowsS)
    return;
end
...
gpsObsTypes = [ObsType.POS_E_X, ObsType.POS_E_Y, ObsType.POS_E_Z];
for i = 1:3
    [obsDb, ok] = obsDb.add(gpsObsTypes(i), x_e_meas(i), x_e_est(i), 1 / dTtag);
    if ok
        H(obsDb.obsCount, :) = H_tmp(i, :);
        obsTypes(end + 1) = double(gpsObsTypes(i)); %#ok<AGROW>
    end
end
end

function [obsDb, H, obsTypes] = prep_nhc_update_with_types(obsDb, H, x, P, measDb, conf, sMap, esMap)
obsTypes = zeros(1, 0);
...
if ~needToRejectVelLat
    ...
    if ok
        ...
        obsTypes(end + 1) = double(ObsType.VEL_C_Y); %#ok<AGROW>
    end
end
if ~needToRejectVelVer
    ...
    if ok
        ...
        obsTypes(end + 1) = double(ObsType.VEL_C_Z); %#ok<AGROW>
    end
end
end

function types = canonical_obs_types(rawObsTypes)
if isempty(rawObsTypes)
    types = zeros(1, 0);
    return;
end
types = zeros(1, numel(rawObsTypes));
for i = 1:numel(rawObsTypes)
    switch rawObsTypes(i)
        case 0
            types(i) = 1;
        case 1
            types(i) = 2;
