...
#[test]
fn full_short_fixture_matches_golden_observation_sequence_and_checkpoints() {
    let fixture_dir =
        PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("tests/fixtures/full_nsr_short");
    let scratch_dir = std::env::temp_dir().join(format!("full_parity_{}", std::process::id()));
    ...
    let status = Command::new(run_full_nsr_path())
        .arg("--input-dir")
        .arg(&fixture_dir)
        .arg("--init-json")
        ...
        .status()
        .expect("run_full_nsr");
    assert!(status.success(), "run_full_nsr failed with status {status}");
...
fn run_full_nsr_path() -> PathBuf {
    let exe_dir = std::env::current_exe()
        ...
        .expect("target dir")
        .to_path_buf();
    let mut candidate = exe_dir.join("run_full_nsr");
    if cfg!(windows) {
        candidate.set_extension("exe");
    }
    assert!(
        candidate.is_file(),
        "run_full_nsr binary not found at {}",
        candidate.display()
    );
    candidate
}
