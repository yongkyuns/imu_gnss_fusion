...
fn main() -> Result<()> {
    ...
    let mut noise = ProcessNoise::reference_nsr_demo();
    ...
    for event in events {
        match event.event_type {
            ...
            EventType::Gyro => {
                ...
                let nhc_active = !args.disable_nhc && nhc_gate(full.nominal(), curr, &latest_accel);
    ...
    if time_s.is_empty() {
        bail!("local full replay produced no running samples");
    }
