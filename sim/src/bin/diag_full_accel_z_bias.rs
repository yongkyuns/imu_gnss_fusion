...
#[derive(Parser, Debug)]
#[command(name = "diag_full_accel_z_bias")]
struct Args {
    #[arg(
        long,
        value_delimiter = ',',
        default_value = "sim/motion_profiles/city_blocks_15min.scenario,sim/motion_profiles/figure8_15min.scenario,sim/motion_profiles/straight_accel_brake_15min.scenario,sim/motion_profiles/sloped_start_recovery_15min.scenario"
    )]
    ...
    for imu_index in 1..generated.imu.len() {
        ...
        if let Some(truth) = generated.truth.get(imu_index) {
            ...
            final_row = ResultRow {
                ...
                final_baz_sigma_mps2: full.covariance()[11][11].max(0.0).sqrt() as f64,
