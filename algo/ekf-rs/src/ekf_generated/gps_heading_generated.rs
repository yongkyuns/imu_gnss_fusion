...
// Sub Expressions
let HK0 = q0*q3 + q1*q2;
let HK1 = 2.0*powf(q2, 2.0) + 2.0*powf(q3, 2.0) - 1.0;
let HK2 = 4.0*powf(HK0, 2.0) + powf(HK1, 2.0);
let HK3 = 2.0/HK2;
let HK4 = HK1*q3;
let HK5 = HK1*q2;
let HK6 = -4.0*HK0*q2 + HK1*q1;
let HK7 = -HK6;
let HK8 = -4.0*HK0*q3 + HK1*q0;
let HK9 = -HK8;
let HK10 = HK4*P[0][0] + HK5*P[0][1] - HK7*P[0][2] - HK9*P[0][3];
let HK11 = HK4*P[0][1] + HK5*P[1][1] - HK7*P[1][2] - HK9*P[1][3];
let HK12 = 4.0/powf(HK2, 2.0);
let HK13 = HK4*P[0][3] + HK5*P[1][3] - HK7*P[2][3] - HK9*P[3][3];
let HK14 = HK4*P[0][2] + HK5*P[1][2] - HK7*P[2][2] - HK9*P[2][3];
let HK15 = HK3/(HK10*HK12*HK4 + HK11*HK12*HK5 + HK12*HK13*HK8 + HK12*HK14*HK6 + R_YAW);
